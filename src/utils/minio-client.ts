...
export async function ensureBucket(): Promise<void> {
  try {
    ...
    // Always (re)apply public-read so EXISTING buckets also serve objects without
    // signed links. Previously this only ran on first creation, leaving existing
    // buckets private.
    const policy = {
      Version: "2012-10-17",
      Statement: [
    ...
    log.info("MinIO bucket policy ensured (public-read)", { bucket: BUCKET_NAME });
  } catch (error) {
